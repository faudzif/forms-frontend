(function () {
	...
	const CONFIG = {
		API_BASE: "https://forms-backend-h3jq.onrender.com/api",
		ENDPOINTS: {
			FORM: "/form",
		},
	};
      ...
      `;
			...
			actions: {
				...
				updateAndDeleteHandler(wrapper, field) {
					...
					fieldActionButtons.className = `field-action-buttons flex gap-1 w-full mt-4 ${appFormBuilder.state.isEditModeEnabled ? "" : "hidden"
						}`;
					...
					inputs.forEach((input) => {
						...
						if (input.tagName === "TABLE") {
							if (
								...
								field.radioName = `${appFormBuilder.state.fieldType
