(function () {
  ...
  const appFormBuilder = {
    element: {
      ...
      formBuilderWrapper: document.getElementById("formBuilder"),
      formPreview: document.getElementById("form-preview"),
      clearFormButton: document.getElementById("clearFormBtn"),
      fieldTypeDropdown: document.getElementById("FieldTypeDropDown"),
      regexField: document.getElementById("regexField"),
      regexType: document.getElementById("RegexType"),
      editButtonsSet: document.getElementById("edit-buttons-set"),
      noFieldsEl: document.getElementById("no-fields"),

      // Single Elements
      editModeBtn: document.querySelector(".edit-mode-btn"),
      cancelEditModeBtn: document.querySelector(".cancel-edit-mode-btn"),

      // Multiple Elements
      formLeftPanel: document.querySelectorAll(".form-submit"),
      closeButtonsFieldBox: document.querySelectorAll(".closeBtnFieldBoxJS"),
      regexValidationBox: document.querySelectorAll(".regex-box"),
    ...
    helpers: {
      ...
      setValidationConfig(field, input, type, value, onSubmit) {
        ...
        if (appFormBuilder.state.isFieldEditing) {
          ...
          const resolvedValue = onSubmit
            ? parseInt(appFormBuilder.utils.getSiblingValue(input, 1)?.value) ||
              null
      ...
      tableItemsHandler(table, type, field, onSubmit) {
        if (appFormBuilder.state.isFieldEditing) {
          return onSubmit
            ? appFormBuilder.helpers.tableItemsSubmitHandler(field, table, type)
            : appFormBuilder.actions.bindTableItemsFromJSON(field, table, type);
        }
      ...
      applyInputAttributes(input, field, lang, wrapper) {
        ...
        appFormBuilder.helpers.applyValidationMsg(input, field, lang, wrapper);
      ...
      handleRegexSubmit(field, input, validationValue, checkbox, customInput) {
        if (!input.checked) {
      ...
      attachFileUploadHandler() {
        setTimeout(() => {
          appFormBuilder.helpers.createEnhancedFileUploadHandler("attachment");
        }, 100);
      },
        ...
        field.items?.forEach((item) => {
          ...
          if (field.type === "checkBoxes") {
            container.appendChild(
              appFormBuilder.helpers.createToggle(itemId, labelText, id, field)
            );
          } else {
            container.appendChild(
      ...
      renderButtonSubmit(field, lang, id, wrapper, label, form, buttonsLength) {
      ...
      renderButtonCancel(field, lang, id, wrapper, label, form, buttonsLength) {
      ...
      showNoFieldsMessage() {
        const hasFields = appFormBuilder.state.formFields.length > 0;
    ...
    actions: {
      ...
      showFieldBox(selectedValue) {
        ...
        if (map[selectedValue]) {
          document
            .getElementById(map[selectedValue])
      ...
      updateAndDeleteHandler(wrapper, field) {
        ...
        // Append to wrapper
        if (appFormBuilder.state.formFields.length > 1) {
        ...
        deleteWrapper.addEventListener("click", () => {
          appFormBuilder.API.deleteField(fieldId);
        });
      },
    ...
    handlers: {
      ...
      handleCloseFieldBoxClick(event) {
        ...
        appFormBuilder.utils.addNRemoveClassFromAll(".saveBtn", "hidden", true);
      ...
      editModeHandler() {
        ...
        appFormBuilder.element.editModeBtn.addEventListener("click", () => {
          fieldActionButtons = document.querySelectorAll(
    ...
    core: {
      ...
      createJSONFromField(e, editSelectedSection, onSubmit) {
        ...
        if (!appFormBuilder.state.isFieldEditing) {
          appFormBuilder.state.formFields.push(field);
      ...
      bindFieldData(field) {
        ...
        if (appFormBuilder.state.editFieldId) {
          ...
          appFormBuilder.state.activeFieldForm = document.getElementById(
            appFormBuilder.state.editFieldType
          );
          const saveBtn =
      ...
      renderFormPreview() {
        ...
        appFormBuilder.element.formPreview.dir = lang === "ar" ? "rtl" : "ltr";

        if (!appFormBuilder.element.formPreview) return;
        appFormBuilder.element.formPreview.innerHTML = "";

        const submitBtnCount = appFormBuilder.state.formFields.filter(
          (item) => item.type === "submitButton"
        ).length;
        const cancelButttonCount = appFormBuilder.state.formFields.filter(
          (item) => item.type === "cancelButton"
        ).length;

        const buttonsLength = submitBtnCount + cancelButttonCount;

        appFormBuilder.state.formFields.forEach((field) => {
          const wrapper = document.createElement("div");
      ...
      enableFieldReordering() {
        new Sortable(appFormBuilder.element.formPreview, {
          ...
          onEnd: function (evt) {
            ...
            const oldOrder = appFormBuilder.state.formFields.map((f) => f.id);
            const isSameOrder =
              oldOrder.length === newOrder.length &&
              oldOrder.every((id, idx) => id === newOrder[idx]);

            if (!isSameOrder) {
              appFormBuilder.state.formFields =
    ...
    API: {
      ...
      async loadForm() {
        try {
          ...
          const res = await fetch(`${CONFIG.API_BASE}${CONFIG.ENDPOINTS.FORM}`);
          if (!res.ok) throw new Error(`Failed to fetch form: ${res.status}`);

          const data = await res.json();

          // update state instead of global "fields"
          appFormBuilder.state.formFields = data;

          // call preview and edit handlers if available
          appFormBuilder.core.renderFormPreview?.();
          appFormBuilder.handlers.editModeHandler?.();
        } catch (err) {
        ...
        try {
          ...
          const res = await fetch(
            `${CONFIG.API_BASE}${CONFIG.ENDPOINTS.FORM}`,
            {
              ...
              body: JSON.stringify(appFormBuilder.state.formFields),
            }
          );
      ...
      async deleteField(id) {
        try {
          ...
          const res = await fetch(
            `${CONFIG.API_BASE}${CONFIG.ENDPOINTS.FORM}/${id}`,
            {
              method: "DELETE",
            }
          );
          ...
          const data = await res.json();
          appFormBuilder.state.formFields = data.savedForm || [];
      ...
      async updateFieldAPIHandler(id, newData) {
        try {
          const res = await fetch(
            `${CONFIG.API_BASE}${CONFIG.ENDPOINTS.FORM}/${id}`,
            {
    ...
    init() {
      ...
      this.element?.closeButtonsFieldBox?.forEach?.((btn) => {
        btn?.addEventListener?.("click", this.handlers?.handleCloseFieldBoxClick);
      });
      ...
      this.element?.clearFormButton?.addEventListener?.(
        "click",
        this.API?.deleteAllFields
      );
    },
  };
